...
状态：M0–M7 已完成；首个 post-v1 真实仓库试点已完成

日期：`2026-07-17`

当前阶段：首个隔离本地试点已完成并脱敏归档；下一步由独立 finding 或第二个异构样本驱动。
...
首个脱敏结果见 `docs/pilots/atypical-life-club-2026-07-17.md`：它验证了当前工具基线在
固定输入上的安全闭环、byte-stable 重扫、stale 检测、关系抽样和两个任务投影，但不扩大
任何 framework 或运行时支持声明。
...
## 18. 下一步：在证据驱动下选择后续样本或能力切片

首个固定样本已经验证了现有版本的安全闭环与有限任务价值。下一步不自动扩展分析器，
而是依据脱敏结果选择一个新的、独立的工作单元：

1. 若要外推实用性，选择一个语言、框架或仓库形态不同的固定样本，并复用同一隔离与
   脱敏协议；
2. 若遇到可独立复现的漏报、误报、性能或安全 finding，先开窄 issue 冻结契约，再决定
   是否进入新的能力里程碑；
3. 保持试点结论与产品支持矩阵分离，直到有设计过的多样本评测。

这个阶段不通过单一样本倒推“所有仓库已支持”，也不把未评测的 Hugo、媒体、部署或
运行时行为包装为确定性事实。
